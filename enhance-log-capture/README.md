...
- `ARCHIVE_ENABLE` - Enable or disable moving rotated files to archive directory (default: `1` = enabled, `0` = disabled). When enabled, rotated log files are moved to `ARCHIVE_DIR` after rotation. When disabled, files remain in `ACTIVE_DIR`.
